...
function HMathP.FUNC(str)
	...
	function(str)
		...
		for equat in str:gmatch('%b[]') do 
			equat = equat:sub(2, -2);
			-- for arg in equat:gmatch('.-%,') do 
				-- table.insert(Arr, HMathP.ProcessEquation(arg:sub(1, -2)));
				-- equat = equat:sub(arg:len() + 1);
			-- end;
		...
		local Args = Arr;
		-- local Arr = HMathP.ExtractNumbers(str);
		-- local Eval = HMathP.ProcessEquation(equat);
		if name == 'sqrt' then 
...
while true do 
	local eq = io.read();
	local result;
	local status, msg = pcall(function() result = HMathP.ProcessEquation(eq); end);
	if not status then 
		print('ERROR: '.. msg);
	else
